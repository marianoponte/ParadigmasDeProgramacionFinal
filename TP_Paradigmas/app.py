...
from forms import ClienteForm, ProductoForm, LoginForm, RegistrarForm, CargarVentaForm
		...
		with open('registros.csv') as archivo:
			...
			while registro:
				...
				try:
					...
					preciostr = str(registro[dicindices['indice_precio']])
					if registro[dicindices['indice_precio']].find(".") == -1 or registro[dicindices['indice_precio']].find(".") == 0 or preciostr[-1] == ".":
						valido = False
...
# Funcion para validar los datos cargados para ingresar una nueva venta
def validarcarga(codigo,cantidad,precio):
	patron = re.compile(r"^[A-Z]{3}\d{3}$")
	valido = True
	errordevalidacion = set([])
	if patron.search(codigo) == None:
		valido = False
		errordevalidacion.add("Codigo de producto invalido")
	try: 
		int(cantidad)
	...
	try:
		...
		print(preciostr)
		if precio.find(".") == -1 or precio.find(".") == 0 or preciostr[-1] == ".":
			valido = False
			errordevalidacion.add("Columna precio no tiene un valor decimal")
	except:			#Aca no entra nunca ya que la validacion se hace en el try, por eso pongo pass.
		pass
...
def cargarventa():
	if 'username' in session:
		...
		if formulario.validate_on_submit():
			...
			validacion = validarcarga(codigo,cantidad,precio)
			if validacion[0] == True:
				with open('registros.csv','a',newline='') as archivo:
					archivo_csv = csv.writer(archivo)
					dicindices = indices()
					registro = [0,0,0,0,0]	#Inicializo una lista "registro" en cero y cargo las columnas segun su orden de indices
					registro[dicindices['indice_codigo']] = formulario.codigo.data
					registro[dicindices['indice_producto']] = formulario.producto.data
					registro[dicindices['indice_cliente']] = formulario.cliente.data
					registro[dicindices['indice_cantidad']] = formulario.cantidad.data
					registro[dicindices['indice_precio']] = formulario.precio.data
					archivo_csv.writerow(registro)	#Escribo en el archivo la columna
					flash('La venta se ha cargado correctamente')
					return render_template('cargaexitosa.html')
			else:
				return render_template('errorcarga.html',error=validacion[1])
		return render_template('cargarventa.html', form=formulario)
	else:
		return render_template('sin_permiso.html')
